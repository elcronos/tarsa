...
None of this provides value. The event log is for deriving ClaudeLens UI state, not for auditing OS process history.
...
- There is no built-in way to list "sessions spawned by ClaudeLens" from the event log. If tracking is needed in the future, a lightweight polling mechanism (e.g., `tmux list-sessions`) is preferred over retrofitting event-sourcing to process lifecycle.
