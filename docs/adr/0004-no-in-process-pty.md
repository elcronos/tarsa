...
A recurring feature request is an in-browser terminal for spawning new Claude Code sessions directly from the ClaudeLens UI. Two implementation paths were considered:

1. **In-process PTY via xterm.js + node-pty:** Embed a terminal emulator in the frontend; relay PTY I/O over WebSocket through the ClaudeLens server process.
...
In-process PTY is rejected. ClaudeLens uses external multiplexer spawn: `POST /api/spawn` creates a detached tmux session and returns the `tmux attach` command. The user attaches in their own terminal.

See ADR-0006 for why `/api/spawn` is implemented as an action endpoint rather than an event-sourced operation.

A future `claudelens-shell` sister project may explore browser-embedded terminal UX for users who need it.
